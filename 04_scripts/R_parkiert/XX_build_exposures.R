...
#' excluded) by absolute exam-date difference (can be before or after), 
#' pulls FFQ-derived dairy variables and time-varying covariates from that matched wave, 
#' then computes cumulative average dairy intake.
#'
...
build_exposures <- function(colaus_derived,
    ...
    colaus_tv <- colaus_derived |>
        dplyr::select(
            ...
            # HGS_peak into handgrip_max_all in build_sarcopenia() after this step)
    ...
    .baseline_only_vars <- c("cdv_event", "hrt_status", "education_level")
