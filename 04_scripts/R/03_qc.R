...
# troubleshooting table can include key columns (pt, cohort, wave, wave_num, Age, sex)
...
#'   `wave_num`, `Age`, `sex`, and `exam_date_iso`.
#'
#' @return A data frame (qc_tbl) with one row per participant per wave, containing:
#'   - Key identifiers: `pt`, `cohort`, `wave`, `wave_num`, `Age`, `sex`, `exam_date_iso`
...
qc <- function(harmonised_list) {
    ...
    cli::cli_inform(c(
        ...
        "x" = "Duplicate entries in wave: {qc_summary$n_qc_pt_unique}",
