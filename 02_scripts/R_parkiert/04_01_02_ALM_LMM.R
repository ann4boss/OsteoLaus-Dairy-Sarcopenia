...
visualize(mod_no_slope, formula = ALM_HT2_harmonised  ~ time_since_baseline + pt , sample = 5, plot = "model")

p <- visualize(mod_no_slope, 
               formula = ALM_HT2_harmonised ~ time_since_baseline + pt, 
               sample = 5, 
               plot = "model")
...
p + 
  labs(x = "Time since baseline [years]",
       y = expression("AMLI"~"[kg/m"^2~"]")) + theme(panel.grid = element_blank()) + 
  geom_point(size = 3, alpha = 1) + theme(panel.grid = element_blank(),
                                        panel.border = element_blank(),  # Removes entire border
                                        axis.line = element_line(color = "black"),  # Keep left and bottom axes
                                        axis.line.top = element_blank(),  # Explicitly remove top axis line
                                        axis.line.right = element_blank(),  # Explicitly remove right axis line
