...
derive_bmi_category <- function(df) {
    ...
    report_stats <- df |>
        dplyr::summarise(
            ...
            # obese or overweight
            n_over25 = sum(BMI_category %in% c("Overweight", "Obese"), na.rm = TRUE),
            .groups = "drop"
        ) |>
        dplyr::as_tibble()
    ...
    cli::cli_inform(c(
        ...
        " " = "Prevalence of Overweight + Obesity: {round(report_stats$n_over25 / (report_stats$n_total - report_stats$n_miss) * 100, 1)}%"
        ))
